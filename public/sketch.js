...
function setup() {
  ...
  for (var i = 0; i < lunok - 1; i++) {
   ...
   for (var i = 0; i <= 10 * 2 - 1; i++) {
    ...
    if(i < 19){
      plinkos.push(new Plinko(width - rastPlinkoW / 2 + rastPlinkoW / 4,plus - rastPlinkoH / 2, radiusPlinko, 'red'));
      plinkos.push(new Plinko(rastPlinkoW / 2 - rastPlinkoW / 4,plus - rastPlinkoH / 2, radiusPlinko, 'red'));
    }

    for (var j = pos_sm; j <= width - 0; j=j+rastPlinkoW) 
    {
      plinkos.push(new Plinko(j,plus, radiusPlinko, 'white'));
    }
  }
