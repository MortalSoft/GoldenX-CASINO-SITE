			...
			<div class="flex no_padding wrap">
				<div class="col">
					<div class="flex no_padding wrap">
						...
						<div class="col-lg-4">
							<div class="content-area " style="min-height: 10px;">
								<button class="btn-auth w-100" >Топ рефоводов по рефералам</button>
							</div>
						</div>
					</div>
...
<script type="text/javascript">
	function getUser() {
		$.post('/admin/user/top_refs_ref',{_token: csrf_token}).then(e=>{
			$('#all_users').html('')
			$('#button_menu').html('')

			paginate = JSON.parse(e.paginate);
			for (var i = 0; i < paginate.length; i++) {
				$('#button_menu').append('<button onclick="pageList('+(Number(paginate[i]) - 1)+', `userAllTopRef`, getUser)" class="btn-dep w-100 '+(Number(paginate[i]) - 1)+'">'+paginate[i]+'</button>')
			}
